    ...
    const pageSubtitle = (
        <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap', color: 'var(--text-muted)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <PlatformIcon platform={event.platform || 'spotify'} size={16} />
                {(() => {
                    ...
                    return url ? (
                        <a
                            ...
                            onMouseEnter={e => e.currentTarget.style.color = event.platform?.toLowerCase() === 'youtube' ? '#FF0000' : '#1DB954'}
                            onMouseLeave={e => e.currentTarget.style.color = 'var(--text-muted)'}
                        >
                            {event.playlist_name}
                        </a>
                    ) : <span>{event.playlist_name}</span>;
                })()}
            </div>
        </div>
    );
